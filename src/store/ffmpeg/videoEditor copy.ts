...
export const useVideoEditorStore = defineStore('videoEditor', () => {
  ...
  // 片段操作
  const markSegmentStart = (time: number) => {
    currentSegmentStart.value = time;
  };
  ...
  const generateBatchCmd = () => {
    if (!segments.value.length || !videoFile.value) return '';

    segments.value.forEach((_, idx) => {
      if (segments.value[idx].enableCrop) {
        validateCropParams(idx);
      }
    });
...
export const useSegmentStore = defineStore('segment', () => {
  ...
  const addSegment = ({ startTime, endTime } : TimeRange) => {
